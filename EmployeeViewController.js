EmployeeViewController = BaseViewController.extend( {
	...
	onTabShow: function( e, ui ) {
		...
		else if (this.edit_view_tab_selected_index === 13) {
    if ($this.current_edit_record && $this.current_edit_record.id) {
        $this.edit_view_tab.find('#tab_attestation_form_content .first-column-sub-view').css('display', 'block');

        $this.edit_view_tab.find('#download-attestation-form').off('click').on('click', function () {
            const record = $this.current_edit_record;

			
			
			console.log("START RECORD DUMP");
			console.log(record);
			console.log("END  RECORD DUMP");
            // Fill the hidden template with actual data
            $('#print_employee_name').text(`${record.first_name} ${record.last_name}`);
            $('#print_employee_birth_place').text(record.city || '----------');
            $('#print_employee_birth_date').text(record.birth_date || '----------');
            $('#print_employee_title').text(record.title || '----------');
            $('#print_employee_start_date').text(record.hire_date || '----------');
            $('#print_employee_salary').text(record.salary || '----------');
            $('#print_employee_salary').text(record.salary || '----------');
  $('#print_employee_regnumber').text(record.other_id1 || '----------');
            // Get the HTML content from the hidden template
            const content = $('#attestation-form-template').html();
		...
		// English version
$this.edit_view_tab.find('#download-attestation-form-en').off('click').on('click', function () {
    const record = $this.current_edit_record;

    // Fill the hidden template with actual data
    $('#print_employee_name_en').text(`${record.first_name} ${record.last_name}`);
    $('#print_employee_birth_place_en').text(record.city || '----------');
    $('#print_employee_birth_date_en').text(record.birth_date || '----------');
    $('#print_employee_title_en').text(record.title || '----------');
    $('#print_employee_start_date_en').text(record.hire_date || '----------');
    $('#print_employee_salary_en').text(record.salary || '----------');
    $('#print_employee_regnumber').text(record.other_id1 || '----------');

    // Get the HTML content from the hidden template
    const contentEn = $('#attestation-form-template-en').html();

    // Open print window
    const printWindowEn = window.open('', '', 'width=800,height=600');
    printWindowEn.document.write(`
        <html lang="en" dir="ltr">
            <head>
                <meta charset="UTF-8">
                <title>Work Attestation</title>
                <style>
                    body {
                        font-family: 'Arial', sans-serif;
                        margin: 40px;
    ...
    printWindowEn.close();
        ...
        $this.edit_view_tab.find('#tab_attestation_form_content .first-column-sub-view').css('display', 'none');
        $this.edit_view.find('.save-and-continue-div').css('display', 'block');
    }
}
	...
	onFormItemChange: function( target, doNotValidate ) {
		...
		switch ( key ) {
			...
			case 'last_name':
				...
				break;
			case 'arabic_first_name':
				var widget = this.edit_view_ui_dic['arabic_first_name'];
				widget.setValue(target.getValue());
				this.current_edit_record.arabic_first_name = target.getValue();
				break;
			case 'arabic_middle_name':
				var widget = this.edit_view_ui_dic['arabic_middle_name'];
				widget.setValue(target.getValue());
				this.current_edit_record.arabic_middle_name = target.getValue();
				break;
			case 'arabic_last_name':
				var widget = this.edit_view_ui_dic['arabic_last_name'];
				widget.setValue(target.getValue());
				this.current_edit_record.arabic_last_name = target.getValue();
				break;			
			case 'first_name_1':
	...
	setTabStatus: function() {
		if ( this.is_mass_editing ) {
			...
			$(this.edit_view_tab.find('ul li a[ref="tab_attestation_form"]')).parent().hide();
			this.edit_view_tab.tabs( 'select', 0 );

		} else {

			$(this.edit_view_tab.find('ul li a[ref="tab_attestation_form"]')).parent().show();
		...
		for ( var key in this.current_edit_record ) {
			...
			if ( Global.isSet( widget ) || key === 'hierarchy_control' ) {
				switch ( key ) {
					...
					console.log("Cur");
					console.log(this.current_edit_record[key] );
						widget.setValue( this.current_edit_record[key] );
						break;
					case 'country':
...
//					$this.grid.addRowData( new_record.id, new_record, 0 );
	...
	initTabData: function() {
        ...
        if (this.current_edit_record && this.current_edit_record.id) {
            this.edit_view_tab.find('#tab_attestation_form_content .first-column-sub-view').css('display', 'block');
            // If you have any init function for this tab, call it here.
            // For example: this.initSubAttestationFormView();
        } else {
            this.edit_view_tab.find('#tab_attestation_form_content .first-column-sub-view').css('display', 'none');
            this.edit_view.find('.save-and-continue-div').css('display', 'block');
        }
	...
	onMapClick: function() {
		...
		var data = {filter_columns: {id:true, first_name:true, last_name:true, arabic_first_name:true, arabic_middle_name:true, arabic_last_name:true, address1:true, address2:true, city:true, province:true, country:true, postal_code:true, latitude:true, longitude:true}};
	...
	buildEditViewUI: function() {
		...
		this.setTabLabels( {
			...
			'tab_attestation_form': $.i18n._( 'Attestation Form' )
		} );
...
//		this.addEditFieldToColumn( $.i18n._( 'Company' ), form_item_input, tab_employee_column1, '' );
		...
		// Arabic First Name
		form_item_input = Global.loadWidgetByName(FormItemType.TEXT_INPUT);
		form_item_input.TTextInput({ field: 'arabic_first_name', width: 200 });
		this.addEditFieldToColumn($.i18n._('Arabic First Name'), form_item_input, tab_employee_column2, '');
		
		// Arabic Middle Name
		form_item_input = Global.loadWidgetByName(FormItemType.TEXT_INPUT);
		form_item_input.TTextInput({ field: 'arabic_middle_name', width: 200 });
		this.addEditFieldToColumn($.i18n._('Arabic Middle Name'), form_item_input, tab_employee_column2, '');
		
		// Arabic Last Name
		form_item_input = Global.loadWidgetByName(FormItemType.TEXT_INPUT);
		form_item_input.TTextInput({ field: 'arabic_last_name', width: 200 });
		this.addEditFieldToColumn($.i18n._('Arabic Last Name'), form_item_input, tab_employee_column2, '');
...
console.log('form_item_input:', form_item_input);

form_item_input.TImage({
...
this.addEditFieldToColumn(
    $.i18n._('Profile Picture'),
    form_item_input,
    tab_employee_column2
);
...
*/
	...
	//			new ServiceCaller().uploadFile( target.getValue(), 'object_type=', {onResult: function( result ) {
    //
	...
	//			new ServiceCaller().uploadFile( form_data, 'object_type=', {onResult: function( result ) {
    //
	//				if ( result.toLowerCase() === 'true' ) {
	//					//$this.file_browser.setImage( ServiceCaller.companyLogo );
	//					$this.UpdateProfilePicture();
...
//
//    // Extract the file from form_data (assumes it's under 'filedata' or similar)
//    const fileData = form_data.get('profile_picture_url'); // 'filedata' is the default name in your widget
//    if (!fileData || !(fileData instanceof File)) {
//        TAlertManager.showAlert('No file selected', 'Error');
//        return;
//    }
//
//    realFormData.append('file_data', fileData); // Our upload script expects 'file'
//
//    new ServiceCaller().customUploadFile(
//        realFormData,
//        'views/employees/employee/profile_picture_upload.php',
//        {
//            onResult: function(result) {
...
//                    } catch (e) {
//                        TAlertManager.showAlert('Invalid server response', 'Error');
//                        return;
//                    }
//                }
//
//                if (result.success) {
//                    TAlertManager.showAlert('Upload successful', 'Success');
//                    console.log(result.file_url);
//                    // You can call: this.setImage(result.file_url);
//                } else {
//                    TAlertManager.showAlert(result.message || 'Upload failed', 'Error');
//                }
//            }
//        }
//    );
...
//
//
//		this.addEditFieldToColumn( $.i18n._( 'Profile Picture' ), this.file_browser, tab_employee_column2, '', null, false, true );
...
//    allowed_extensions: ['jpg', 'jpeg', 'png', 'gif'],
//    max_file_size: 5 * 1024 * 1024,
//    upload_path: 'employee/profile_pictures/',
//    file_types_description: 'Image Files'
//});
//
//this.addEditFieldToColumn(
//    $.i18n._('Profile Picture TXT'),
//    form_item_input,
//    tab_employee_column2,
//    '',
//    null,
...
//);
		...
		//}
        //
		//this.addEditFieldToColumn( $.i18n._( 'Photo' ), this.file_browser, tab_employee_column2, '', null, false, true );
...
if (typeof FormData == "undefined") {
    ...
    this.file_browser.bind('imageChange', function (e, target) {
        new ServiceCaller().uploadFile(target.getValue(), 'object_type=user_photo&object_id=' + $this.current_edit_record.id, {
            onResult: function (result) {
                if (result.toLowerCase() === 'true') {
                    var imgUrl = ServiceCaller.userPhoto + '&object_id=' + $this.current_edit_record.id;
                    $this.file_browser.setImage(imgUrl);
                    addButtons();
                } else {
                    TAlertManager.showAlert(result, 'Error');
                }
...
this.addEditFieldToColumn($.i18n._('Photo'), this.file_browser, tab_employee_column2, '', null, false, true);
 	...
 	//Print
		...
		this.downloadbuttonfield = form_item_input.TTagInput( {field: 'printId', object_type_id: 200} );
		this.addEditFieldToColumn( $.i18n._( 'Print ID Card' ), this.downloadbuttonfield, tab_employee_column2, '', null, null, true );
		
		
    var PrintIDBtn = $('<button type="button" class="t-button btn btn-sm btn-danger" style="margin-left:5px;">Print ID</button>');
    PrintIDBtn.on('click', function () {
       // $this.file_browser.setImage('');
        //btnContainer.remove();
		openIDCard();
		
    });
	 var btnContainer = $('<div class="IDCARD-action-buttons" style="margin-top:5px;"></div>');
	   var container1 = this.downloadbuttonfield;
    btnContainer.append(PrintIDBtn);
    container1.append(btnContainer);
	...
	function openIDCard() {
    try {
        var rec = $this.current_edit_record ? JSON.parse(JSON.stringify($this.current_edit_record)) : {};
        rec.photo_url = (typeof ServiceCaller !== 'undefined' && ServiceCaller.userPhoto)
            ? (ServiceCaller.userPhoto + '&object_id=' + ($this.current_edit_record ? $this.current_edit_record.id : ''))
            : '';
        rec.display_name = [rec.first_name, rec.middle_name, rec.last_name].filter(Boolean).join(' ').trim();

        var form = document.createElement('form');
        form.method = 'POST';
        form.action = 'card/card.php';
        form.target = '_blank';

        var input = document.createElement('input');
        input.type = 'hidden';
        input.name = 'data';
        input.value = JSON.stringify(rec);
        ...
        console.error('ID Card open error:', e);
        alert('Unable to open ID Card: ' + e);
    }
}
...
function addButtons() {
   var btnContainer = $('<div class="photo-action-buttons" style="margin-top:5px;"></div>');
	   var container = $this.file_browser;

    // Remove existing buttons to avoid duplicates
    container.find('.photo-action-buttons').remove();
   ...
   // var btnContainer = $('<div class="photo-action-buttons" style="margin-top:5px;"></div>');
    ...
    // Download button
    var downloadBtn = $('<button type="button" class="t-button btn btn-sm btn-primary">Download</button>');
    downloadBtn.on('click', function () {
        var imgUrl = ServiceCaller.userPhoto + '&object_id=' + $this.current_edit_record.id;
        var link = document.createElement('a');
        link.href = imgUrl;
        link.download = $this.current_edit_record.id + '.img';
    ...
    // Clear button
    var clearBtn = $('<button type="button" class="t-button btn btn-sm btn-danger" style="margin-left:5px;">Clear</button>');
    clearBtn.on('click', function () {
        $this.file_browser.setImage('');
        btnContainer.remove();
    });
    btnContainer.append(downloadBtn).append(clearBtn);
	...
	buildSearchFields: function() {
		...
		this.search_fields = [
			...
			new SearchField( {
				label: $.i18n._( 'Arabic First Name' ),
				field: 'arabic_first_name',
				basic_search: true,
				in_column: 1,
				form_item_type: FormItemType.TEXT_INPUT
			} ),	
			new SearchField( {
				label: $.i18n._( 'Arabic Middle Name' ),
				field: 'arabic_middle_name',
				basic_search: true,
				in_column: 1,
				form_item_type: FormItemType.TEXT_INPUT
			} ),	
			new SearchField( {
				label: $.i18n._( 'Arabic Last Name' ),
				field: 'arabic_last_name',
				basic_search: true,
				in_column: 1,
				form_item_type: FormItemType.TEXT_INPUT
			} ),
